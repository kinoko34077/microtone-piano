      ...
      <div className="fixed top-0 right-0 h-full w-[320px] bg-[#0d1117] border-l border-[#30363d] shadow-2xl z-50 flex flex-col overflow-y-auto overflow-x-hidden text-slate-200">
        <div className="sticky top-0 bg-[#0d1117] border-b border-[#30363d] p-3 flex justify-between items-center z-10">
          <div className="flex flex-col">
            ...
            <p className="text-[10px] text-slate-400">Microtonal keyboard settings</p>
          </div>
        ...
        <div className="p-4 flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-[#30363d] pb-1">Audio</h3>
            ...
            >
              <VolumeX size={18} />
              <span className="text-xs font-bold">All Notes Off</span>
            </button>

            <label className="text-xs font-bold text-slate-200 mt-2">Sound Source</label>
            <select
            ...
            <div className="mt-2 flex flex-col gap-1">
              <div className="flex items-center justify-between text-xs font-bold text-slate-200">
                <span>Decay</span>
                <span className="font-mono text-sky-300">{noteDecayMs === 0 ? 'off' : `${noteDecayMs} ms`}</span>
              </div>
              <input
                type="range"
                min="0"
          ...
          <div className="flex flex-col gap-2">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-[#30363d] pb-1">Presets</h3>

            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-bold text-slate-400">Layout</label>
                ...
                >
                  {allLayouts.map((layout) => (
                    <option key={layout.id} value={layout.id}>
                      {layout.isStandard ? `[std] ${layout.name}` : layout.name}
                    </option>
                  ))}
                </select>
            ...
            <div className="flex flex-col gap-1 mt-1">
              <label className="text-[10px] font-bold text-slate-400">Tuning</label>
              <div className="flex gap-1">
                <select
                ...
                >
                  {allTunings.map((tuning) => (
                    <option key={tuning.id} value={tuning.id}>
                      {tuning.isStandard ? `[std] ${tuning.name}` : tuning.name}
                    </option>
                  ))}
                </select>
          ...
          <div className="flex flex-col gap-3">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-[#30363d] pb-1">Keyboard</h3>

            <div className="flex flex-col gap-1">
              <label className="text-xs text-slate-200">Label Mode</label>
              <select
          ...
          <div className="flex flex-col gap-2">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-[#30363d] pb-1">Invalid Sections</h3>
            <label className="flex items-center gap-2 cursor-pointer text-xs font-bold text-slate-200">
              ...
              Show invalid sections
            </label>
            <label className="text-xs font-bold text-slate-200 mt-1">Invalid mode</label>
            <select
          ...
          <div className="flex flex-col gap-2 pb-10">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-[#30363d] pb-1">PC Keyboard</h3>
            <div className="flex flex-col gap-1">
              <div className="flex justify-between text-xs">
